...
- **`decisions/`:** Several technologies here already have a dedicated ADR (PostgreSQL — 0003; Redis — 0004; React Native — 0006; Next.js — 0007; React/Vite Admin Dashboard — 0008; Object Storage/S3 — 0016; primary-key identifiers — 0019). This document does not repeat those ADRs' Decision/Rationale; it cross-references them and adds material the ADR format doesn't carry well — a fuller alternatives comparison, and an explicit future-replacement strategy. Technologies with no dedicated ADR yet (NestJS, JWT, OTP, Cloud Infrastructure, CDN) are documented here in full and flagged as backfill candidates, consistent with the pattern already established when ADRs 0002–0017 were backfilled.
...
**Future replacement strategy:** the module structure NestJS expresses today is an architectural requirement (Principles 4.4–4.7) independent of the framework — a future replacement, whether a different Node framework or the extraction of an individual module into its own service (ADR-0002/0009's designed reversibility), would preserve the capability-module boundaries themselves and change only the underlying framework or runtime executing them. No dedicated ADR exists yet for NestJS specifically (ADR-0005 covers the TypeScript-centered stack broadly); this is flagged as a backfill candidate consistent with the pattern already used for ADRs 0002–0017.
...
**Future replacement strategy:** CDN choice is a delivery-layer optimization sitting in front of already-portable origins (S3 objects addressed by key; Next.js-rendered pages) — swapping CDN providers is a DNS and configuration change plus a cache-warm period, not a change to any business logic or data model. It can be revisited freely as pricing, performance, or regional-coverage evidence warrants, without an architectural redesign. No dedicated ADR exists yet; flagged as a backfill candidate alongside NestJS, JWT, OTP, and Cloud Infrastructure's provider-specific detail.

---

## Summary: backfill candidates

Consistent with the pattern already used when ADRs 0002–0017 were backfilled (`decisions/README.md`), the following technologies documented above do not yet have a dedicated ADR and are named here as outstanding candidates: **NestJS** (Section 4), **JWT** (Section 8), **OTP** (Section 9), **Cloud Infrastructure/AWS provider specifics** (Section 10), and **CDN** (Section 11). The remaining technologies are already backed by a dedicated, Accepted ADR, cross-referenced in each section above.
