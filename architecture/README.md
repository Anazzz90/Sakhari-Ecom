...
|   `-- engineering-standards-and-development-guidelines.md   the ESDG — the canonical implementation standard every future SDD cites: tech stack, project/module structure, naming, API/DTO/validation/error/logging/security/database/repository/domain/event/integration standards, testing/performance/code-quality standards, git workflow, CI/CD, documentation standards, Definition of Done, developer checklist, AI development guidelines
|-- 10-templates/
|   `-- software-design-document-template.md   the canonical, generic 24-section SDD template every module's own SDD is copied from, so all sixteen module SDDs share one identical structure
...
| `04-cross-cutting/integration-and-messaging.md` | Event Architecture: the transactional outbox that makes domain-event delivery durable (ADR-0029), event naming/publish/consume rules, background processing, the full notification flow, outbox-aware retry/cross-instance/ordering rules, idempotency, event versioning, external integration resilience per provider (ADR-0032), and the event-transport path from in-process today to a broker on module extraction. Synchronous API convention consolidation remains open (see its own Open Decisions). |
| `04-cross-cutting/observability-and-operations.md` | Operational architecture for a solo operator: logging strategy, three-tier monitoring (system/application/business health), the metrics catalog, health checks, four-tier alerting, five-phase incident response, backup/DR operational practice, RPO/RTO framework per data class, runbook categories, maintenance windows, capacity monitoring, and a production readiness checklist. |
...
| `06-quality-attributes/nfr-matrix.md` | The master NFR reference: seventeen quality attributes (Performance through Compliance), each with description, architectural goal, business importance, supporting ADRs, measurement/acceptance criteria, risks, trade-offs, and future considerations, plus consolidated targets/objectives sections, a Risk Assessment, and an Architecture Traceability matrix. Carries ADR-0031's launch numeric targets throughout. |
| `07-coding-standards/coding-standards.md` | Repository/module structure, naming conventions, logging/validation/error-handling standards, testing philosophy, documentation standards, module boundary enforcement (static analysis, CI validation, architecture tests), a code review checklist, and a separate architecture-compliance checklist. Expands `00-Architecture-Principles.md` Section 8 into concrete, checkable practice. |
...
| `09-engineering-standards/engineering-standards-and-development-guidelines.md` | The ESDG — the canonical implementation standard every future SDD must cite and comply with. Twenty-four sections covering technology stack, project/module structure, naming, API/DTO/validation/error-handling/logging/security/database/repository/domain-model/event/external-integration standards, testing, performance, code quality, git workflow, CI/CD, documentation standards, a definition of done, a developer checklist, and AI development guidelines — normative throughout, using RFC 2119 terminology, citing rather than repeating the architecture documents and ADRs it operationalizes. |
| `10-templates/software-design-document-template.md` | The canonical Software Design Document template — a fixed 24-section structure (Document Information through Completion Checklist) every one of the sixteen modules' own SDDs is produced from, so every module SDD looks identical. Generic throughout: each section states what belongs there and which SRD/DDD/ADR/architecture/ESDG source it's drawn from, never module-specific content itself. Cites the ESDG instead of repeating its standards. |
...
12. **Engineering Standards & Development Guidelines** (`09-engineering-standards/`) — written only once the Architecture Phase itself is complete: it is the implementation-standard layer between this folder and per-module SDDs, and cites every document above rather than re-deciding any of them.
13. **SDD Template** (`10-templates/`) — written last of all, after the ESDG exists to cite: the canonical, generic structure every future module SDD is copied from, so the sixteen module SDDs to come share one identical shape.
...
| `09-engineering-standards/engineering-standards-and-development-guidelines.md` (normative rules stable — each is a direct expression of an Accepted ADR or architecture document; tool-specific detail, e.g. linter/test-framework names, evolves as those below-architecture choices are made) | |
| `10-templates/software-design-document-template.md` (the 24-section structure is stable — changing it reshapes every existing module SDD at once, so it changes only with the same deliberateness as an ADR) | |
...
6. **Check `09-engineering-standards/engineering-standards-and-development-guidelines.md` (the ESDG)** before writing a controller, DTO, migration, event, or test — it is the concrete, checkable implementation standard every module's code and SDD must satisfy; never re-derive a convention it already states.
7. **Use `10-templates/software-design-document-template.md`** as the starting point for any module's own SDD — copy it, complete every section, never invent a different structure or omit a section.
8. **Never edit an Accepted ADR's Decision section, and never restructure `/architecture` itself, without the user's explicit direction** — this folder is a governance artifact; an assistant should propose a new ADR or flag a conflict rather than quietly resolving it by rewriting history.
9. **When a document here is still a skeleton** (see **Current status**), treat its "Sections to be authored" list as the scope of what still needs deciding, and prefer asking or proposing an ADR over silently inventing the missing architecture.
10. **Do not treat this folder as optional context.** For any task that touches system structure — as opposed to a local bug fix or a UI tweak fully inside one already-defined unit — these documents are the constraints layer the SRD and DDD alone cannot provide.

---

## Current status

**Every document in `/architecture` is fully authored, and the first Architecture Readiness Review (ARR) has been remediated.** The Constitution, the ADS, all six numbered detail folders (`02-context/` through `06-quality-attributes/`), `07-coding-standards/`, `08-ai-development/`, and the full `decisions/` set (0001–0034) are current. ADRs 0029–0033 closed the ARR's nine findings — event durability (transactional outbox), the checkout transaction ambiguity between ADR-0010 and ADR-0021, numeric production-readiness targets (latency, availability, RPO/RTO, alert thresholds), external-integration resilience per provider, the RBAC permission catalogue, session management parameters, concurrency/locking order, module-boundary enforcement tooling, and audit durability — and every affected document was updated to carry the resolved decision rather than leaving it ADR-only. ADR-0034 extends the Delivery module with multi-order delivery batching, resolving the extension point DDD Section 17.4 and SRD BR-017 had deliberately left open — no new module, no change to any other module's ownership or transaction boundary.

This does not mean the documentation set is "finished" in the sense of never changing again. Several documents still carry genuinely open, narrower items (module-ownership edge cases, tooling package choices, accessibility standards, a formal legal/compliance review) — see each document directly, `08-ai-development/ai-development-rules.md` Section 14 for a consolidated pointer, and `06-quality-attributes/nfr-matrix.md` Section 14 for the master list. Authoring per-module SDDs and beginning implementation are the natural next phases of work. See `CHANGELOG.md` for the full history of how this folder was built, including the ARR remediation entry.

**The Architecture Phase is complete.** `09-engineering-standards/engineering-standards-and-development-guidelines.md` (the ESDG) has since been authored as the implementation-standard layer between this folder and per-module SDDs — it redesigns nothing, introduces no new business requirement, module boundary, or ADR-level decision, and is the document every future SDD must cite and comply with. `10-templates/software-design-document-template.md` has since been authored as the canonical, generic SDD structure — it designs no module, only the fixed 24-section shape every future module SDD is copied from and completed against.
