...
- **Inventing SLA numbers, backup retention windows, or scaling thresholds that differ from ADR-0031/ADR-0030.** These are no longer open — `reliability-and-performance.md` Section 3, `infrastructure-and-release.md` Sections 9–10, and `06-quality-attributes/nfr-matrix.md` Sections 4–6 all carry the same ADR-0031 numbers. A plausible-sounding but different number ("99.99% uptime," "90-day retention") is still an invention if it doesn't match the approved figure.
- **Assuming a specific test framework, linter, or monorepo/polyrepo structure** — `coding-standards.md` Section 14 explicitly leaves all of these open (Section 12's module-boundary enforcement approach itself, however, is decided — only the specific lint package is open). Do not generate configuration or code that presumes a tool choice without flagging the assumption.
...
This document introduces no Open Decisions of its own — it inherits and points to every other document's. The ARR (ADRs 0029–0033) closed the largest gaps that used to be listed here: event durability, checkout transaction wording, numeric readiness targets, external-integration resilience, RBAC/session/refund/Settings/geocoding/POS ambiguity. What genuinely remains open, checking the cited document directly rather than assuming: Support's finer-grained ticket permissions and rider OTP/Delivery interface wiring (`security-and-compliance.md` Section 12); archival timing and retention-window specifics (`data-architecture.md` Section 18); synchronous API convention consolidation and the outbox dispatcher's exact claim mechanism (`integration-and-messaging.md` Section 13); exact VPC/subnet allocation and monitoring/alerting tool choice (`infrastructure-and-release.md` Section 14, `observability-and-operations.md` Section 17); horizontal-scaling/read-replica trigger thresholds and partitioning specifics (`reliability-and-performance.md` Section 13); and test framework, linter, and repository topology, including the specific import-restriction tool for module boundary enforcement (`coding-standards.md` Section 14).
