...
### External Integrations
- **Moyasar Payment Gateway** — authorization and capture across mada, card, Apple Pay, and BNPL where supported; owned by the Payments capability (ADR-0022), following ADR-0032's timeout/retry/circuit-breaker rules. Cash and card-on-delivery (collected by the Rider through the Worker App, per the DDD's Cash Remittance and Card-on-Delivery Record entities) are reconciled inside the Backend/Infrastructure and do not require this integration to complete an order (ADR-0010, clarified by ADR-0021). Per ADR-0032/ADR-0033, card-on-delivery terminal/POS settlement reconciliation is Payment-owned manual import plus discrepancy review at MVP; a payment terminal provider's settlement API is an optional future enhancement, not a committed external integration in this document today.
- **Unifonic SMS/OTP Provider** — authentication OTPs, rider delivery-confirmation OTPs, and SMS delivery/status notifications; owned by the Identity & Access capability (OTP) and the Notifications capability (delivery/status messages, delivery-confirmation OTP) respectively (ADR-0023), following ADR-0032's resilience rules.
- **Push Notification Service** — outbound push delivery to installed mobile clients, dispatched by the Notifications capability, following ADR-0032's resilience rules. Confirmed for the Customer Mobile App by the DDD's Notification entity. Per ADR-0033, Worker App push notifications are confirmed for task assignment and operational alerts, with polling as the standing fallback (ADR-0032). Browser notifications for the Customer Web App and email are both noted in the DDD as future channels, not current ones.
- **Geocoding / Maps Provider** — address validation and geocoding metadata for customer delivery addresses (DDD Section 5.3, "Address validation depends on Google Maps/geocoding"), following ADR-0032's resilience rules. Per ADR-0033, customer clients may use the Google Maps client SDK for autocomplete only; the Backend performs authoritative geocoding validation, coordinate persistence, service-zone resolution, and the order-time address snapshot — geocoding is backend-mediated for every authoritative purpose, consistent with every other external integration (Principle 4.6).
...
With the one narrow, ADR-0033-confirmed exception (the Google Maps client SDK, reachable directly from the Customer Platform for autocomplete only — never for authoritative validation) and the one inbound-only exception (push delivery), no external system in this table is ever reachable from the Customer Platform or the Operations Platform. Every external edge in the system originates from the Backend or, for hosting only, from Infrastructure. A payment terminal/POS settlement provider for card-on-delivery reconciliation is intentionally **not** included here — per ADR-0032/ADR-0033, MVP launch uses Payment-owned manual reconciliation instead, and a terminal provider integration remains a future enhancement, not a committed integration today.
